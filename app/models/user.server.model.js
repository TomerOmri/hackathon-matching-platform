...
UserSchema.statics.updateUserVote = function(userId, teamId, userVote, cb) {
    ...
    this.findOne({_id: userId}, (err, user) => {
        console.log(`Voting for team ${params.teamId}`);
        let teamVotes = user.team_votes || "";
        let teamVotesObj;
        if (teamVotes) {
            teamVotesObj = JSON.parse(teamVotes);
        } else {
            teamVotesObj = {};
        }
        alreadyVoted = (teamVotesObj[params.teamId]) ? {isVoted: true, lastVote: teamVotesObj[params.teamId]} : {isVoted: false, lastVote: 0};
        teamVotesObj[params.teamId] = params.userVote;
        user.team_votes = JSON.stringify(teamVotesObj);
        user.save((err) => {
            if (err) {
                cb(err);
            } else {
                cb(null, params.userVote, alreadyVoted)
            }
        });
    });
};
